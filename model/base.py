...
class BaseModel():
  ...
  def evaluate(self, eval_data):
    logging.info('Loading model from %s', self.hparams.ckpt)
    model = tf.keras.models.load_model(self.hparams.ckpt)
    model.compile(optimizer=self.get_optimizer(),
                  loss=self.get_loss(), metrics=self.get_metrics())
    metrics = model.evaluate(eval_data, verbose=2)
    logits = model.predict(eval_data)
    logits_file = os.path.join(self.hparams.ckpt, 'logits.npy')
